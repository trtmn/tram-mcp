...
describe("configView", () => {
  ...
  it("labels the credential source 'local' for env/credstore creds", () => {
    expect(configView(FULL_ENV).credential_source).toBe("local (env or ~/.tram-mcp)");
  });

  it("labels the credential source 'per-client' when props are supplied", () => {
    const view = configView({} as Env, { testrailUrl: "https://x", testrailUsername: "u", testrailApiKey: "k" });
    expect(view.credential_source).toBe("per-client");
  });
});
